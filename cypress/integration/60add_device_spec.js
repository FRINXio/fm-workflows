...
describe('Add a device to inventory and mount it', function() {
  ...
  it('Create a new cli device in the inventory', function() {
    ...
    */
    ...
    cy.get('g > rect').eq(1).should('exist')
    cy.get('g > rect').eq(1)
  ...
  it('Create a new cli device in the inventory REPEAT to test CONDITION', function() {
    cy.server()
    cy.route('POST', '/api/conductor/workflow').as('getWorkflowId')
  ...
  it('Create a new netconf device in the inventory', function() {
    ...
    // here we must click no to type !!! cy.get('@tcponly').type(tcponly)
    cy.get('@tcponly').contains(tcponly).click()
    ...
    // here we must click no to type !!! cy.get('@uniconfignative').type(uniconfignative)
    cy.get('@uniconfignative').contains(uniconfignative).click()
