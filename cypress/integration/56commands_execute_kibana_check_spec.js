...
describe('Check that executed command is stored in the inventory', function() {
  ...
  it('retrieves data', function() {
    ...
    cy.contains('table', 'IOS01').closest('table').as('Table')
    cy.get('@Table').contains('sh_run')

    cy.contains('table', 'XR01').closest('table').as('Table')
    cy.get('@Table').contains('sh_run')

    cy.contains('table', 'Leaf02').closest('table').as('Table')
    cy.get('@Table').contains('sh_run')

    cy.contains('table', 'Spine02').closest('table').as('Table')
    cy.get('@Table').contains('sh_run')
